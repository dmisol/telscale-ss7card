  ...
  for(linknumber=0;linknumber<linkqty;linknumber++){
	...
	if(((sltT2[linknumber]>sltT2value)&&Wait4slta[linknumber])/*||(sltT2>sltT2longvalue)*/){
		if(Wait4slta[linknumber] < sltQty){
			...
			for(int i=0;i<15;i++)
				ss7msg->body.su.user.mtp.management.slt.pattern[i] = testpattern1[i];
	...
	if((sltT1[linknumber]>sltT1value)&&(Wait4slta[linknumber]>=sltQty)){	
		...
		log("link timeout",1,(char*)&linknumber);
...
/*
	if((ss7msg->body.su.mtp3.bitfield.dpc) != (rxlabel[linknumber]&0x3FFF) ){
		//if(debug&ABNORMAL_TRAFFIC){ 
		    log("stp functions not supported",5,&(ss7msg->body.board.byte[3]));
		    i = ss7msg->body.su.mtp3.bitfield.dpc;
		    log("addressed as (dpc) ",2,(char*)&i);
		//}
		return 0;}
*/
		/* up to here */
	...
	if( (ss7msg->body.su.mtp2.bitfield.si) == SLTX_SI ){
		if(ss7msg->body.su.user.mtp.h0h1 == SLTM){
			//if(debug&MANAGEMENT_TRAFFIC) 
			//	log("sltm ",5,&(ss7msg->body.board.byte[3]));
#ifdef SPC_AUTODETECT
			/* SPC/NI autodetect */
...
//					(ss7msg->body.su.mtp2.bitfield.ssf)>>2);
			...
			//if(debug&MANAGEMENT_TRAFFIC)
			//	log("slta ",5,&(ss7msg->body.board.byte[3]));
			//if((debug&ESSENTIAL_EVENTS)&&(linkstatus[linknumber]==0)) 
				log("link IT",1,(char*)&linknumber);
	...
	if( (ss7msg->body.su.mtp2.bitfield.si) == NETMNG_SI ){
		switch (ss7msg->body.su.user.mtp.h0h1){
			case TFP:
				//if(debug&MANAGEMENT_TRAFFIC)
					log("TFP",2,(char *)&(ss7msg->body.su.user.mtp.management.spc));
				spc = (ss7msg->body.su.user.mtp.management.spc[0]<<8) + ss7msg->body.su.user.mtp.management.spc[1];
				post_DUNA(spc);
				break;
			case TFA:	
				//if(debug&MANAGEMENT_TRAFFIC)
					log("TFA",2,(char *)&(ss7msg->body.su.user.mtp.management.spc));
			...
			case COA:		// ChangeOver Ack - newer expected
				//if(debug&(MANAGEMENT_TRAFFIC|ABNORMAL_TRAFFIC))
					log("COA (?)",0,0);
				break;
			case CBD:
				linknumber = p->src;
			...
			default:	
				//if(debug&MANAGEMENT_TRAFFIC)
					log("MTP network management",2,(char *)&(ss7msg->body.su.user.mtp.management.spc));
				break;
		}
		return 0;
	}
	...
	//if(debug&ABNORMAL_TRAFFIC)
		log("unknown subsystem",5,&(ss7msg->body.board.byte[3]));
	return 0;
  ...
  case SS7_LINKPAUSE:
	...
	//if((debug&ESSENTIAL_EVENTS)) 
		log("link OOS",1,(char*)&linknumber);
